...
async function getArtist(id: string) {
  ...
  // Try by name (URL-encoded or slug)
  try {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${artist.name} | Nightup`,
    description: artist.about?.slice(0, 155) ?? `${artist.name} on Nightup.`,
  };
}
  ...
  return (
    <div style={{ backgroundColor: "#0F0F1A", minHeight: "100vh" }}>
      ...
      <div className="relative w-full overflow-hidden" style={{ height: "clamp(320px, 45vw, 540px)" }}>
        {artist.photo ? (
          <Image src={artist.photo} alt={artist.name} fill sizes="100vw" className="object-cover object-top" priority />
        ) : (
          <Image src={FALLBACK} alt={artist.name} fill sizes="100vw" className="object-cover" priority />
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 max-w-5xl mx-auto px-4 pb-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-none mb-1">{artist.name}</h1>
        </div>
      </div>
