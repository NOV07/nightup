...
export default function TonightModal({ spots }: { spots: Spot[] }) {
  ...
  return (
    <>
      ...
      {!open ? null : (
        <div style={S.scrim} onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}>
          <div style={S.modal}>
            {/* atmosphere */}
              ...
              {view === "tiles" && (
                <div>
                  ...
                  <button onClick={() => setMoodOpen(true)} style={S.surprise}
                    ...
                    <span>Έκπληξέ με<br/><span style={{ fontWeight: 400, fontSize: 10.5, opacity: 0.7 }}>Ολόκληρη βραδιά σε 2 taps</span></span>
                  </button>
                  <div style={{ textAlign: "center", marginTop: 16 }}>
                    <button onClick={closeModal} style={S.skip}>Όχι τώρα — δες την αρχική →</button>
                  </div>
                </div>
              )}
