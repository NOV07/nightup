"use client";

import Image from "next/image";
import EventImageFallback from "./EventImageFallback";

export default function EventHeroImage({ imageUrl, title, genre, venue, date }: { imageUrl: string | null; title: string; genre?: string; venue?: string; date?: string }) {
  return (
    <div style={{ position: "relative", width: "100%", height: "clamp(260px, 42vw, 440px)", overflow: "hidden", background: "linear-gradient(to bottom, #1a1a2e 0%, #0F0F1A 100%)" }}>
      {imageUrl ? (
        imageUrl.startsWith("data:") ? (
          <img
            src={imageUrl}
            alt={title}
            style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }}
          />
        ) : (
          <Image
            src={imageUrl}
            alt={title}
            fill
            sizes="100vw"
            priority
            style={{ objectFit: "cover" }}
          />
        )
      ) : (
        <EventImageFallback genre={genre} venue={venue} date={date} />
      )}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(15,15,26,0.2), rgba(15,15,26,0.85))" }} />
    </div>
  );
}
